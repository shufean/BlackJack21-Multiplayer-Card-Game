...
class ManageConnection implements BlackJackStatus
{
	...
	public ManageConnection (BlackJackServer game)
	{
		this.game = game;
	...
	public void connect(ServerSocket serverSocket)
	{
		try
		{
			...
			if (game.getPlayersCount() < BlackJackStatus.MAXPLAYERS )
			{
				...
				if ((inputLine = in.readUTF())!= null)
				{
					...
					BlackJackPlayer blackJackPlayer = new BlackJackPlayer(username,playerSocket);
					if (!game.readstatus())
					game.write_clientlist(username);
					if (game.addWaitingPlayers(blackJackPlayer))
					{	
