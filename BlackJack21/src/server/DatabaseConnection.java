...
{	
	...
	public static void setIntoUserTable(int userID, String value) {
		...
		try {
			...
			catch (Throwable ignore) { /*  Propagate the original exception instead of this one that you may want just logged */
			}
		}
	}
		...
		Connection conn = connect();
			...
			try {
				...
				stmt = conn.createStatement();
					...
					stmt.execute("INSERT into cards set username='"+username+"', card_type = '"+card+"', value = '"+value+"'");
