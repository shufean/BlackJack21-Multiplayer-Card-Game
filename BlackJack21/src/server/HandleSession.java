...
class HandleSession extends Thread implements GameStatus
{
	...
	private Vector<BlackJackCard> cards ;
	public HandleSession(BlackJackServer game) throws IOException
	{
		...
		{
			System.out.println(players.get(i).getSocket().getInetAddress().toString()+" "+players.get(i).getUsername());
		...
		cards = new Vector<BlackJackCard>();
	...
	public void writehand(int index) throws IOException
	{
		...
		game.write_clienthand(players.get(index).getUsername(), players.get(index).getHand());
	...
	////////////////
				...
				else
				{
					...
					for (int i=0 ; i<game.getPlayersCount()+1;i++)
					{
						cards = game.read_clienthand(players.get(i).getUsername());
